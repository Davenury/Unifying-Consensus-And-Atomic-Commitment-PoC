...
fun startApplication(
    ...
    embeddedServer(Netty, port = 8080 + conf.portOffset, host = "0.0.0.0") {
                ...
                val timer =
                        ProtocolTimerImpl(
                                config.protocol.leaderFailTimeoutInSecs,
                                config.protocol.backoffBound
                        )
                val protocolClient = ProtocolClientImpl()
                val transactionBlocker = TransactionBlockerImpl()
                val otherPeers =
                        getOtherPeers(config.peers.peersAddresses, conf.nodeId, conf.peersetId)
                val protocol =
                        GPACProtocolImpl(
                                historyManagement,
        ...
        try {
            peersAddresses.foldIndexed(mutableListOf()) { index, acc, strings ->
                if (index == peersetId - 1) {
                    acc +=
                            strings.filterNot {
                                it.contains("peer$nodeId") || it.contains("${basePort + nodeId}")
                            }
                    acc
                } else {
                    acc += strings
