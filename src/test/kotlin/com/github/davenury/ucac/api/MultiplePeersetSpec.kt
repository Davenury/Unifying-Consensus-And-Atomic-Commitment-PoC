...
class MultiplePeersetSpec {
    ...
    fun `should be able to execute change in two different peersets even if changes in peersets are different`() =
        runBlocking {
            ...
            expectCatching {
                executeChange("http://${peers[0][0]}/v2/change/sync?enforce_gpac=true", someChange)
            }.isSuccess()
            ...
            expectCatching {
                executeChange(
                    "http://${peers[1][0]}/v2/change/sync?enforce_gpac=true",
            ...
            expectCatching {
                executeChange("http://${peers[0][0]}/v2/change/sync?enforce_gpac=true", addRelationChange)
            }.isSuccess()
