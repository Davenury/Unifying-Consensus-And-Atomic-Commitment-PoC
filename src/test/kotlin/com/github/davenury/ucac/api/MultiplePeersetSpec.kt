...
class MultiplePeersetSpec {
    ...
    fun `should execute transaction in every peer from every of two peersets`(): Unit = runBlocking {
        ...
        val change = change(otherPeer, InitialHistoryEntry.getId())
            ...
            val leaderAction = SignalListener {
                ...
                runBlocking {
                    testHttpClient.post<HttpResponse>(url2) {
                        ...
                        accept(ContentType.Application.Json)
                        body = Apply(
                            it.transaction!!.ballotNumber, true, Accept.COMMIT, AddUserChange(
    ...
    private fun change(otherPeersetPeer: String, parentId: String) = AddUserChange(
        parentId,
        "userName",
