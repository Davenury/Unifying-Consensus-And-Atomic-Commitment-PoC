...
import github.davenury.ucac.common.AddGroupChange
...
import github.davenury.ucac.gpac.domain.*
...
class SinglePeersetIntegrationTest {
    ...
    fun `second leader tries to become leader before first leader goes into ft-agree phase`(): Unit = runBlocking {
        ...
        val peers = apps.map { "http://localhost:${it.getBoundPort()}" }
        ...
        expectThrows<ServerResponseException> {
            executeChange("${peers[0]}/create_change")
        }
